...
''' Usage: pagerank.py input.csv
    output: top 50 ranked nodes by pagerank index
'''

def read_csv(filename):
    reader = csv.DictReader(open(filename), fieldnames=("s", "t"))
    list = []
    for row in reader:
        list.append((row["s"], row["t"]))
    return list
...
def main(argv):
    graph = read_csv(argv[1])
    # print graph
    # print "\n"
    ranks, _ = pagerank(graph)
    print sorted(ranks.items(), key=(lambda d: d[1]), reverse=True)[:50]
